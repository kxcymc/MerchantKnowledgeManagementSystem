...
function checkPopplerAvailable() {
  try {
    ...
    // 首先尝试直接运行命令（适用于所有平台，如果已添加到 PATH）
    try {
      execSync('pdftoppm -v', { 
    ...
    if (isMacOS) {
      ...
      try {
        ...
        // 如果成功，更新 process.env.PATH 以便后续 spawn() 调用继承
        const curPath = process.env.PATH || '';
    ...
    if (isWindows) {
      const windowsCandidates = [
        ...
        path.join(process.env['ProgramFiles(x86)'] || 'C:\\Program Files (x86)', 'poppler', 'Library', 'bin', 'pdftoppm.exe')
      ];
...
async function convertPdfPageWithPoppler(filePath, page, outputDir, outputPrefix) {
  return new Promise((resolve, reject) => {
    ...
    // 根据平台确定命令名称
    // 注意：在 Windows 上，如果 Poppler 在 PATH 中，通常不需要 .exe 扩展名
    // 但如果直接指定路径，则需要 .exe 扩展名
    const platform = process.platform;
    const isWindows = platform === 'win32';
    const command = 'pdftoppm'; // 使用通用命令名，让系统在 PATH 中查找
    
    const pdftoppm = spawn(command, [
      '-png',
      '-r', '200',  // 200 DPI
      '-f', page.toString(),
      '-l', page.toString(),
