...
const splitter = new RecursiveCharacterTextSplitter({
  chunkSize: 400,  // 从800减小到400
  chunkOverlap: 80, // 从160减小到80
  separators: ['\n\n', '\n', '。', '！', '？', ' ']
});

// 智能语义分割器（支持自适应chunk大小、跨页分割）
// 减小chunk size以提高检索精度，避免大片数据
const semanticSplitter = new SemanticSplitter({
  minChunkSize: 150,   // 最小chunk大小（字符数），小于此值会合并
  maxChunkSize: 600,   // 最大chunk大小（字符数），大于此值会拆分（从1200减小到600）
  targetChunkSize: 400, // 目标chunk大小（从800减小到400，更细粒度）
  chunkOverlap: 80      // chunk重叠大小（从160减小到80）
});
