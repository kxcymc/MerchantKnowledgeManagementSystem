  ...
  try {
    ...
    if (current.type === 'json') {
      ...
      try {
        ...
        const newFileUrl = knowledgeService.getFileUrl(filePath); // 绝对路径
        const storagePath = path.relative(config.uploadDir, filePath).replace(/\\/g, '/'); // 相对路径用于向量存储

        // 删除旧 JSON 文件（如果路径发生变化）
        if (current.file_url && current.file_url !== newFileUrl) {
          // file_url 现在存储的是绝对路径，直接使用
          const oldPath = path.isAbsolute(current.file_url) 
            ? current.file_url 
    ...
    if (file) {
      ...
      // 存储绝对路径
      finalFileUrl = path.resolve(file.path);
...
// GET /api/knowledge - 知识列表查询（支持分页，前端使用）
router.get('/knowledge', async (req, res) => {
  try {
    ...
    // 查询总数
    const allResults = await dbService.queryKnowledge({
      title,
      business,
      scene,
      status,
    ...
    // 分页处理
    const paginatedResults = allResults.slice(offset, offset + pageSizeNum);
    ...
    const formattedResults = paginatedResults.map(item => {
      ...
      if (item.created_at) {
        ...
        createdAtStr = date.toISOString().split('T')[0];
      }
      ...
      const result = {
        ...
        pdf_url: item.file_url || '', // 前端预览使用 pdf_url
        title: item.title,
...
// GET /api/mul-query - 多条件查询知识列表（兼容旧接口）
...
// DELETE /api/delete - 删除知识（通过 body）
...
// DELETE /api/knowledge/:id - 删除知识（通过 URL 参数，前端使用）
router.delete('/knowledge/:id', async (req, res) => {
  try {
    const knowledge_id = parseInt(req.params.id);

    if (!knowledge_id || isNaN(knowledge_id)) {
      return res.status(400).json({ message: 'knowledge_id 无效' });
    }

    await knowledgeService.deleteKnowledge(knowledge_id);

    return res.json({ message: '删除成功', knowledge_id });
  } catch (error) {
    logger.error('删除知识失败', { error: error.message });
    res.status(500).json({ message: error.message });
  }
...
/**
 * GET /api/statistics/zero-hit-questions - 查询零命中问题列表
 * 
 * 查询参数：
 * - limit: 返回数量限制，默认100
 * - startDate: 开始日期（可选，格式：YYYY-MM-DD 或 YYYY-MM-DD HH:mm:ss）
 * - endDate: 结束日期（可选，格式：YYYY-MM-DD 或 YYYY-MM-DD HH:mm:ss）
 * - orderBy: 排序字段，'asked_count' 或 'last_asked_at'，默认 'asked_count'
 * 
 * 返回：零命中问题列表
 */
router.get('/statistics/zero-hit-questions', async (req, res) => {
  try {
    const {
      limit = '100',
      startDate = null,
    ...
    const options = {
      ...
      orderBy: orderBy || 'asked_count'
    };

    const questions = await dbService.getZeroHitQuestions(options);
    ...
    logger.error('查询零命中问题失败', { error: error.message });
    res.status(500).json({
      success: false,
      message: '查询零命中问题失败',
      error: error.message
...
/**
 * GET /api/statistics/hot-knowledge-distribution - 查询热点知识分布
 * 
 * 查询参数：
 * - limit: 返回数量限制，默认50
 * - startDate: 开始日期（可选，格式：YYYY-MM-DD 或 YYYY-MM-DD HH:mm:ss）
 * - endDate: 结束日期（可选，格式：YYYY-MM-DD 或 YYYY-MM-DD HH:mm:ss）
 * 
 * 返回：热点知识分布列表，包含 knowledge_id, citation_count, message_count, session_count 等
 */
router.get('/statistics/hot-knowledge-distribution', async (req, res) => {
  try {
    const {
      limit = '50',
      startDate = null,
    ...
    const distribution = await dbService.getHotKnowledgeDistribution(options);

    // 可选：关联 Knowledge 表获取知识库标题等信息
    const knowledgeIds = distribution.map(item => item.knowledge_id).filter(Boolean);
    ...
    if (knowledgeIds.length > 0) {
      try {
        ...
        logger.warn('获取知识库详情失败', { error: error.message });
      }
    }
    ...
    const result = distribution.map(item => ({
      ...item,
      knowledge: knowledgeMap.get(item.knowledge_id) || null
    }));
    ...
    logger.error('查询热点知识分布失败', { error: error.message });
    res.status(500).json({
      success: false,
      message: '查询热点知识分布失败',
      error: error.message
...
/**
 * GET /api/statistics/hot-professional-questions - 查询热点专业知识问题
 * 
 * 查询参数：
 * - limit: 返回数量限制，默认50
 * - minCount: 最小问题数量，默认2
 ...
 * 返回：热点专业知识问题列表
 */
router.get('/statistics/hot-professional-questions', async (req, res) => {
  try {
    const {
      limit = '50',
      minCount = '2',
    ...
    const questions = await dbService.getHotProfessionalQuestions(options);
    ...
    logger.error('查询热点专业知识问题失败', { error: error.message });
    res.status(500).json({
      success: false,
      message: '查询热点专业知识问题失败',
      error: error.message
    });
  }
});

// ============== 统计分析 API（用于分析页和多维数据分析） ==============

/**
 * GET /api/statistics/overview - 获取数据总览统计
 * 返回：会话数、消息数、知识库数、引用数等基础统计
 */
router.get('/statistics/overview', async (req, res) => {
  try {
    // 获取会话统计
    const [sessionStats] = await dbService.pool.execute(
      `SELECT 
        COUNT(*) as total_sessions,
        COUNT(CASE WHEN DATE(created_at) = CURDATE() THEN 1 END) as today_sessions,
        COUNT(CASE WHEN DATE(created_at) = DATE_SUB(CURDATE(), INTERVAL 1 DAY) THEN 1 END) as yesterday_sessions
      FROM Session WHERE visible = 1`
    );

    // 获取消息统计
    const [messageStats] = await dbService.pool.execute(
      `SELECT 
    ...
    // 获取知识库统计
    const [knowledgeStats] = await dbService.pool.execute(
      `SELECT 
        COUNT(*) as total_knowledge,
        COUNT(CASE WHEN status = '生效中' THEN 1 END) as active_knowledge,
        COUNT(CASE WHEN type = 'pdf' THEN 1 END) as pdf_count,
        COUNT(CASE WHEN type = 'json' THEN 1 END) as json_count
      FROM Knowledge`
    );

    // 获取引用统计
    const [citationStats] = await dbService.pool.execute(
      `SELECT 
        COUNT(*) as total_citations,
        COUNT(DISTINCT knowledge_id) as cited_knowledge_count,
        COUNT(DISTINCT message_id) as cited_message_count
      FROM MessageCitation WHERE knowledge_id IS NOT NULL`
    );

    // 获取最近7天的会话趋势
    const [trendData] = await dbService.pool.execute(
      `SELECT 
    ...
    res.json({
      ...
      data: {
        ...
        citations: citationStats[0] || {},
        trend: trendData || [],
      },
    });
  } catch (error) {
    logger.error('获取数据总览失败', { error: error.message });
    res.status(500).json({
      success: false,
      message: '获取数据总览失败',
      error: error.message,
...
/**
 * GET /api/statistics/daily-activity - 获取每日活动统计
 * 返回：最近7天每天的消息数、会话数等
 */
router.get('/statistics/daily-activity', async (req, res) => {
  try {
    const [data] = await dbService.pool.execute(
      `SELECT 
        DATE(created_at) as date,
    ...
    logger.error('获取每日活动统计失败', { error: error.message });
    res.status(500).json({
      success: false,
      message: '获取每日活动统计失败',
      error: error.message,
...
/**
 * GET /api/statistics/knowledge-by-business - 按业务分类统计知识库
 */
router.get('/statistics/knowledge-by-business', async (req, res) => {
  try {
    const [data] = await dbService.pool.execute(
      `SELECT 
        bs.business,
        COUNT(k.knowledge_id) as count,
        COUNT(CASE WHEN k.status = '生效中' THEN 1 END) as active_count
      FROM BusinessScene bs
      LEFT JOIN Knowledge k ON bs.business_id = k.business_id
      GROUP BY bs.business
      ORDER BY count DESC`
    );
    ...
    logger.error('获取业务分类统计失败', { error: error.message });
    res.status(500).json({
      success: false,
      message: '获取业务分类统计失败',
      error: error.message,
...
/**
 * GET /api/statistics/hourly-distribution - 获取消息时段分布
 */
router.get('/statistics/hourly-distribution', async (req, res) => {
  try {
    const [data] = await dbService.pool.execute(
      `SELECT 
    ...
    logger.error('获取时段分布失败', { error: error.message });
    res.status(500).json({
      success: false,
      message: '获取时段分布失败',
      error: error.message,
