...
async function addFileKnowledge(fileData, isUpdate = false) {
  ...
  if (shouldUpdate && knowledgeId && existingKnowledge) {
    ...
    if (existingKnowledge.file_url && existingKnowledge.type !== 'json') {
      try {
        const oldFilePath = path.join(config.uploadDir, existingKnowledge.file_url.replace(/^\/uploads\//, '').replace(/^uploads\//, ''));
        ...
        if (normalizedOldPath !== normalizedNewPath) {
          ...
          finalFileUrl = existingKnowledge.file_url; // 保持原有的file_url
          ...
          // 路径相同，直接使用
          finalFileUrl = existingKnowledge.file_url;
        }
      } catch (error) {
...
async function addJsonKnowledge(jsonData, isUpdate = false, knowledge_id = null) {
  ...
  if (shouldUpdate && knowledgeId) {
    ...
    if (existing.file_url) {
      try {
        const oldJsonPath = path.join(config.uploadDir, existing.file_url.replace(/^\/uploads\//, '').replace(/^uploads\//, ''));
...
async function deleteKnowledge(knowledgeId) {
  ...
  if (current.file_url) {
    try {
      // file_url 格式: /uploads/相对路径 或 uploads/相对路径
      const relativePath = current.file_url.replace(/^\/uploads\//, '').replace(/^uploads\//, '');
      const filePath = path.join(config.uploadDir, relativePath);
      
      // 检查文件是否存在
      if (await fs.pathExists(filePath)) {
        await fs.remove(filePath);
        logger.info('已删除物理文件', { knowledgeId, filePath: relativePath });
      } else {
