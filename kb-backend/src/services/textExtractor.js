...
async function extractPdfWithPageInfo(filePath) {
  ...
  try {
    ...
    if (ocrText && ocrText.length > 0) {
      // 尝试按页面分割OCR文本（简单的平均分割）
      const lines = ocrText.split('\n');
      const linesPerPage = Math.max(1, Math.ceil(lines.length / pageCount));
      
      lines.forEach((line, index) => {
        if (line && line.trim()) {
          const page = Math.min(pageCount, Math.floor(index / linesPerPage) + 1);
          const lineNum = (index % linesPerPage) + 1;
          pages.push({
            page,
            line: lineNum,
            text: line.trim()
    ...
    if (pages.length === 0 && fullText) {
      const fallbackResult = fullText.split('\n')
        .map((line, idx) => ({ page: 1, line: idx + 1, text: line.trim() }))
